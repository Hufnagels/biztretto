...
var popoverContent2 = function(data){
    if(type !== 'image'){
        switch ( type ) {
            /*case 'image':
        ...
        var sd = data.find('.subdetails');
        var text = '';
        if (sd.find('span' ).length){
            text='<table class="table table-striped">';
            sd.find('span' ).each(function(){
                text += '<tr><td>'+$(this ).attr('class') + '</td><td>' + $(this ).html()+'</td></tr>';
            })
            text+='</table>';
        }
        ...
        img.src = mediaUrl//data('content').match(/src=\"([^\"]+)\"/i)[1];
        img.onload = function(){
            var sd = data.find('.subdetails');
            var text = '';
            if (sd.find('span' ).length){
...
var popoverPlacement = function(source){
...
var popoveroptions = {
    placement: function ( context, source ) {
        return popoverPlacement(source)
    },
...
(function ( MyMedia, $, undefined ) {
    ...
    var myMediaElements = {
            form     : document.getElementById( "mymediaForm" ) || '',
        ...
        toolbarElements = {
            ...
            init : function(){
                ...
                this.sortingElements.init();
            },

            initAddFilesButton: function () {

                addEventO( toolbarElements.addButton, 'click', function ( e ) {
            ...
            sortingElements: {
                ...
                init             : function () {
                    this.$wordFilterButton.bind( 'keyup', function () {
                        ...
                        toolbarElements.sortingElements.sort();
                    } );
...
//console.log( type )
                    ...
                    this.$sortbyButtons.bind( 'click', function ( e ) {
                        e.preventDefault();
                        $( toolbarElements.sortingElements.$sortby ).find( 'button' ).removeClass( 'active' );
                    ...
                    this.$orderButtons.bind( 'click', function ( e ) {
                        e.preventDefault();
                        toolbarElements.sortingElements.$order.find( 'button' ).removeClass( 'active' );
                        $( this ).addClass( 'active' );
                        toolbarElements.sortingElements.sort();
                    } );
                    toolbarElements.sortingElements.sort();
                ...
                sort             : function () {
                    ...
                    var filterArray = [];
                    toolbarElements.sortingElements.$filterButtons.filter(function () {
...
//console.log($(myMediaElements.mediaList).find('li.mediaElement > .colorBar').filter(function(){ return filterArray.indexOf(this.className);}).attr('class'))
        ...
        uploadElements = {
            ...
            init: function () {
                ...
                addEventO( uploadElements.uploadFinishButton, 'click', function ( e ) {
                    if ( $( uploadElements.container ).find( 'li.template-download' ).length == 0 ) {
                        $( toolbarElements.addButton ).trigger( 'click' );
                        return false;
                    }
                ...
                addEventO( uploadElements.addVideoLinkButton, 'click', function ( e ) {
            ...
            setUploadLimit : function(){
                ...
                uploadElements.filesLeft.innerHTML = uploadLimit;
            },
    ...
    var mediaFiles = {
        ...
        setPopover : function(){
            $('li.mediaElement' ).live('click', function(e){
                e.stopPropagation();
                var allowedObjects = ['AUDIO'];
                var obj = (e.target ? e.target : e.srcElement);
                if (obj.tagName.indexOf(allowedObjects) == -1)
                {
                    $('.popover' ).remove().end();// Perform your click action.
                    return false;
                }
            });
            //$(myMediaElements.mediaList).find('.detailButton').clickover(popoveroptions);
            //alert('papp')
            //return false;
            //$(myMediaElements.mediaList).find('span.detailButton').live('click', function (){
            $(myMediaElements.mediaList).on('click', 'span.detailButton', function (){
                var el = this,
                    img = new Image();
                $(el ).popover('destroy');
                /*
                 if($(this).parent().find('.popover').length == 1) {
                 $('.popover' ).remove().end();
                 return false;
                 }
                 */
                mediaFiles.hideAllPopovers();

                if($(el ).closest('li.mediaElement' ).data('category') == 'image') {

                    var sd = $(el ).closest('li.mediaElement' ).find('.subdetails');
                    var text = '';
                    if (sd.find('span' ).length){
                        text='<table class="table table-striped">';
                        sd.find('span' ).each(function(){
                    ...
                    img.src = $(this).closest('li.mediaElement' ).attr('data-mediaurl')//data('content').match(/src=\"([^\"]+)\"/i)[1];
                    img.onload = function(){
                        var placement = popoverPlacement(el);
                        var html = '<img src="'+img.src+'" />' + text;
                        $(el).popover({content: html, html:true, placement:placement, selector : el, title    : function () {
                            return $( this ).closest( 'li.mediaElement' ).attr( 'data-name' ).substring(0,40);
                        }});//
                        $(el).popover('show')
                    };
                } else {
                    ...
                    $(el).popover(popoveroptions);//{content: img, html:true, placement:'right'}
                    //TOlog('video popover', $(el))
                    //return false;

                    $(el).popover('show').on(function(e){
                        $(el).append($('.popover'));
                        ...
                        /*
                         ...
                         mediaFiles.hideAllPopovers(); // prevent event for bubbling up => will not get caught with document.onclick
                         });
                         */
                        //e.stopPropagation();
                    });
    ...
    var mediaGroups = {
        ...
        editGroup : function(entry){
            ...
            $('#bc').html('<span class="pull-left"><a class="editable" id="mbName_'+mgId+'" data-type="text" data-pk="'+mgId+'">' +mgName+'</a></span>'+
                '<span class="btn btn-empty btn-dark" id="deleteGrp" data-id="' + mgId + '"><i class="icon-trash"></i></span>');
    ...
    MyMedia.init = function (quotaString) {
        quota = quotaString;
        ...
        toolbarElements.init();

        uploadElements.init();

        optionElements.init();

        $( uploadElements.container ).slimScroll( { height : '237px', allowPageScroll: false, width: '800px' } );
