...
//TOlog('setSelectedSlide',defaults.templateSlide)
                ...
                addNewSlide : function(returnData){
					...
					if ($(mainElements.slideshowContainer ).find('.dataHolder.selected' ).length) {
						$( tmpl( "tmpl-miniSlides", response, true )).insertAfter( $(mainElements.slideshowContainer).find('.dataHolder.selected' ) )
					
						/**
						 * create helper function from this code
						...
						var response2 = slideshowElements.helperFunctions.handelSlides(entry,4);
						/////////////////////////////////
					*/
					} else {
						$(mainElements.slideshowContainer ).append(tmpl( "tmpl-miniSlides", response, true ))
					}
//alert($(mainElements.slideshowContainer ).find('.dataHolder.selected' ).length )
//return false
...
//console.log( 'image dropped to editor');
                            ...
                            case 'colorBar video':
                                ...
                                switch ( ui.draggable.attr( 'data-mediatype' ) ) {
                                    case 'local':
										var videoLength = ui.draggable.attr( 'data-video-width' ), 
											videoHeight = ui.draggable.attr( 'data-video-height' );
										
										var scale_width = (defaults.editingAreaWidth-cursorX) / videoLength,
											scale_height = (defaults.editingAreaHeight-cursorY) / videoHeight,
											newscale = Math.min(scale_width, scale_height),
											newwidth = videoLength * newscale,
											newheight = videoHeight * newscale;  

                                        eW = ui.draggable.attr( 'data-video-width' ) + cursorX > defaults.editingAreaWidth ? newwidth : ui.draggable.attr( 'data-video-width' );
                                        eH = ui.draggable.attr( 'data-video-height' )+ cursorY > defaults.editingAreaHeight ? newheight : ui.draggable.attr( 'data-video-height' );;
                                        nW = parseInt( eW / defaults.editingAreaWidth * 100 );
                                        nH = parseInt( eH / defaults.editingAreaHeight * 100 );
